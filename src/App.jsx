      ...
      if (user) {
        try {
          ...
          try {
            ...
            console.log('관리자 권한 확인:', { uid: user.uid, isAdmin: adminExists });
    ...
    <div className="min-h-screen bg-navy-50">
      <header className="bg-white shadow-sm sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 sm:px-4 py-3 sm:py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {/* 관리자 링크: admins 컬렉션에 문서가 있는 계정만 표시 */}
            {isAdmin && (
              <a 
                href="/admin.html" 
                className="px-3 py-1.5 bg-navy-700 text-white text-sm rounded-lg hover:bg-navy-800 transition-colors"
              >
                ⚙️ 관리자
              </a>
            )}
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md slide-up">
            <h2 className="text-xl font-bold text-navy-900 mb-2">로그아웃</h2>
            <p className="text-navy-600 mb-6">로그아웃하시겠습니까?</p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowLogoutConfirm(false)}
                className="flex-1 px-4 py-2.5 bg-navy-100 text-navy-700 font-medium rounded-lg hover:bg-navy-200 transition-colors"
              >
                취소
              </button>
              <button
                onClick={confirmLogout}
                className="flex-1 px-4 py-2.5 bg-navy-700 text-white font-medium rounded-lg hover:bg-navy-800 transition-colors"
              >
                확인
              </button>
            </div>
