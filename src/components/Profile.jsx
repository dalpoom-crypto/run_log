...
const Profile = ({ user, userData, runs, relationshipData, currentUser, onViewUserProfile }) => {
  ...
  const [pendingCrewsInfo, setPendingCrewsInfo] = useState({}); // crewId -> crew info
  ...
  const [selectedRun, setSelectedRun] = useState(null); // 상세보기 모달용
  ...
  // 멤버 관리 모달이 열릴 때 멤버 목록 로드
  useEffect(() => {
    const loadCrewMembers = async () => {
      if (!showCrewMembersModal || !myCrew?.id) return;
          ...
          membersSnapshot.docs.map(async (m) => {
            ...
            try {
              ...
              return {
                id: m.id,
                ...data,
                nickname: userData.nickname || userData.displayName || '이름 없음',
              };
            } catch (error) {
    ...
    loadCrewMembers();
  }, [showCrewMembersModal, myCrew?.id]);

  // 크루 가입 신청 알림 클릭 시 멤버 관리 모달만 열기
  useEffect(() => {
    const handleOpenCrewManage = (event) => {
      const { crewId } = event.detail || {};
      if (crewId && myCrew && myCrew.id === crewId && (myCrewRole === 'owner' || myCrewRole === 'admin')) {
        // 멤버 관리 모달만 열기 (멤버 목록은 위의 useEffect에서 자동으로 로드됨)
        setShowCrewMembersModal(true);
      }
    };

    window.addEventListener('openCrewManage', handleOpenCrewManage);
    return () => {
      window.removeEventListener('openCrewManage', handleOpenCrewManage);
    };
  }, [myCrew, myCrewRole]);
  ...
  useEffect(() => {
    const loadScraps = async () => {
      ...
      try {
        ...
        // 권한 오류는 조용히 처리 (규칙이 배포되면 자동으로 해결됨)
  ...
  const handleConfirmJoinCrew = async () => {
    ...
    try {
      ...
      // 크루장에게 알림 생성
      if (crewActionModal.crew.ownerId) {
        try {
          const requesterDoc = await getDoc(doc(db, 'users', user.uid));
          const requesterData = requesterDoc.exists() ? requesterDoc.data() : {};
          await createNotification(crewActionModal.crew.ownerId, 'crewJoinRequest', {
            crewId: crewActionModal.crew.id,
            crewName: crewActionModal.crew.name,
            requesterId: user.uid,
            requesterNickname: requesterData.nickname || user.displayName || '익명',
      ...
      // 가입 신청한 크루 정보 저장
      setPendingCrewsInfo((prev) => ({
        ...prev,
        [crewActionModal.crew.id]: crewActionModal.crew,
      }));
  ...
  const handlePhotoChange = async (e) => {
    ...
    try {
      ...
      // 페이지 새로고침 없이 현재 탭에 머물도록 함
      // user 상태는 onAuthStateChanged에서 자동으로 업데이트됨
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        await createNotification(targetUserId, 'follow', {
          ...
          fromUserNickname: currentUserData.nickname || currentUser.displayName || '익명',
        });
    ...
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
                ...
                {isOwnProfile ? (
                  ...
                  currentUser.uid !== user.uid && (
                    <div className="flex items-center gap-2 sm:gap-3">
                      {/* 나를 일방적으로 팔로우: 맞팔로우 버튼 */}
                      {relationshipData.followingMe && !relationshipData.iAmFollowing && (
                        <button
                          onClick={handleFollow}
                          className="px-2 sm:px-3 py-1 sm:py-1.5 bg-navy-700 text-white text-xs sm:text-sm font-semibold rounded-lg whitespace-nowrap hover:bg-navy-800 transition-colors"
                        >
                          맞팔로우
                        </button>
                      )}
                      {/* 내가 팔로잉 중이면 상태 표시 및 해제 버튼 */}
                      {relationshipData.iAmFollowing && (
                        <button
                          onClick={() => setShowUnfollowSheet(true)}
                          className="px-2 sm:px-3 py-1 sm:py-1.5 bg-navy-100 text-navy-700 text-xs sm:text-sm font-semibold rounded-lg whitespace-nowrap hover:bg-navy-200 transition-colors"
                        >
                          {relationshipData.followingMe ? '러닝 버디' : '팔로잉 중'}
                        </button>
                      )}
                      {/* 팔로우하지 않은 경우: 팔로우 버튼 */}
                      {!relationshipData.iAmFollowing && !relationshipData.followingMe && (
                        <button
                          onClick={handleFollow}
                          className="px-2 sm:px-3 py-1 sm:py-1.5 bg-navy-700 text-white text-xs sm:text-sm font-semibold rounded-lg whitespace-nowrap hover:bg-navy-800 transition-colors"
                        >
            ...
            currentUser.uid !== user.uid && (
              <div className="flex items-center gap-2">
                ...
                {relationshipData.followingMe && !relationshipData.iAmFollowing && (
                  <button
                    onClick={handleFollow}
                    className="px-3 py-1.5 bg-navy-700 text-white text-xs font-semibold rounded-lg whitespace-nowrap hover:bg-navy-800 transition-colors"
                  >
                    맞팔로우
                  </button>
                )}
                ...
                {relationshipData.iAmFollowing && (
                  <button
                    onClick={() => setShowUnfollowSheet(true)}
                    className="px-3 py-1.5 bg-navy-100 text-navy-700 text-xs font-semibold rounded-lg whitespace-nowrap hover:bg-navy-200 transition-colors"
                  >
                    {relationshipData.followingMe ? '러닝 버디' : '팔로잉 중'}
                  </button>
                )}
        ...
        >
          ...
          >
            ...
            <p className="text-sm sm:text-base text-navy-600 text-center mb-6">
              {userData?.nickname || user.displayName || '이 사용자'}님을 더 이상 팔로우하지 않습니다.
            </p>
            <div className="space-y-2">
              <button
                onClick={async () => {
                  try {
                    ...
                    showToast(`${userData?.nickname || user.displayName || '이 사용자'}님을 더 이상 팔로우하지 않습니다.`);
                      ...
                      {scraps.map((scrap) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              // 스크랩한 게시글 상세보기 열기
                              if (scrap.runData && scrap.runId) {
                                // runData에 id 추가
                                const runForDetail = {
                                  ...scrap.runData,
                                  id: scrap.runId
                                };
                                setSelectedRun(runForDetail);
                              }
                            }}
                          >
                            {/* 사진 */}
                ...
                <div>
                  ...
                  {myActivityCrewView === 'main' && (
                    ...
                    <div className="space-y-4">
                                        ...
                                        setConfirmModal({
                                          ...
                                          onConfirm: async () => {
                                            ...
                                            try {
                                              ...
                                              // 크루장에게 탈퇴 알림 생성
                                              if (myCrew.ownerId && myCrew.ownerId !== user.uid) {
                                                try {
                                                  const userDoc = await getDoc(doc(db, 'users', user.uid));
                                                  const userData = userDoc.exists() ? userDoc.data() : {};
                                                  await createNotification(myCrew.ownerId, 'crewMemberLeft', {
                                                    crewId: myCrew.id,
                                                    crewName: myCrew.name,
                                                    memberId: user.uid,
                                                    memberNickname: userData.nickname || user.displayName || '익명',
                                                  });
                                                } catch (notifError) {
                                                  console.error('알림 생성 실패:', notifError);
                                                }
                      ...
                      {/* 크루 검색 + 개설 버튼 (소속된 크루가 없고, 가입 승인 대기 중이 아닐 때만 표시) */}
                      {!myCrew && Object.keys(pendingCrewMemberships).length === 0 && (
                        <div className="space-y-2">
                        <div className="flex gap-2">
                          <input
                            type="text"
                      ...
                      {!myCrew && Object.keys(pendingCrewMemberships).length > 0 && (
                        <div className="max-h-64 overflow-y-auto space-y-3">
                          <p className="text-xs text-navy-500 mb-1">가입 신청한 크루</p>
                          {Object.keys(pendingCrewMemberships).map((crewId) => {
                            const crew = pendingCrewsInfo[crewId] || 
                                       [...recommendedCrews, ...crewSearchResults].find(c => c.id === crewId) || 
                                       recommendedCrews.find(c => c.id === crewId) ||
                                       crewSearchResults.find(c => c.id === crewId);
                            if (!crew) return null;
                            ...
                            return (
                              <div
                                key={crew.id}
                                className="p-3 bg-yellow-50 rounded-xl border border-yellow-200 space-y-2"
                              >
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex justify-between items-center mt-2">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <span className="text-[11px] text-yellow-700 font-semibold bg-yellow-100 px-2 py-0.5 rounded">
                                          승인 대기 중
                                        </span>
                                        <button
                                          onClick={async () => {
                                            ...
                                            try {
                                              ...
                                              if (crew.ownerId) {
                                                try {
                                                  const requesterDoc = await getDoc(doc(db, 'users', user.uid));
                                                  const requesterData = requesterDoc.exists() ? requesterDoc.data() : {};
                                                  await createNotification(crew.ownerId, 'crewJoinCancel', {
                                                    crewId: crew.id,
                                                    crewName: crew.name,
                                                    requesterId: user.uid,
                                                    requesterNickname: requesterData.nickname || user.displayName || '익명',
                      ...
                      {!myCrew && (
                        ...
                        {/* 검색어 없을 때: 추천 크루 (가입 신청한 크루가 없을 때만 표시) */}
                        {!crewSearchLoading &&
                          !crewSearchQuery &&
                          Object.keys(pendingCrewMemberships).length === 0 &&
                                      ...
                                      setConfirmModal({
                                        ...
                                        onConfirm: async () => {
                                          try {
                                            ...
                                            // 거부된 사용자에게 알림 생성
                                            await createNotification(m.userId, 'crewRejected', {
                                              crewId: myCrew.id,
                                              crewName: myCrew.name,
                                            });
      ...
      {selectedRun && (
        <RunDetailModal
          ...
          currentUser={currentUser ? { uid: currentUser.uid, userData: currentUser.userData || {}, displayName: currentUser.displayName || currentUser.userData?.nickname } : null}
