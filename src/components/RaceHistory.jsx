  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        {raceStats.map(({ raceName, bestRecord, latestRecord, count, improvement, allRecords }) => {
          ...
          return (
            <div
              key={raceName}
              className="rounded-lg p-3 sm:p-4 bg-gradient-to-br from-navy-50 to-navy-100 border border-navy-100"
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-navy-900 text-sm sm:text-base truncate">{raceName}</h3>
                  {/* 모바일: 마라톤명 하단에 참가 횟수 표시 */}
                  <div className="mt-1 text-[11px] text-navy-800 sm:hidden">
                    {count}회 참가
                  </div>
                </div>
                {/* 태블릿 이상: 우측에 참가 횟수 표시 */}
                <div className="text-right ml-3 flex-shrink-0 hidden sm:block">
                  ...
                  {allRecords.map((record, index) => (
                    <div key={index} className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-navy-100 text-navy-800 text-xs font-semibold">
