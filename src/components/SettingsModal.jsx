    ...
    try {
      if (nickname !== (userData?.nickname || user.displayName)) {
        try {
          ...
          } else if (checkErr.code === 'unavailable' || checkErr.code === 'deadline-exceeded') {
            errorMessage = '네트워크 연결을 확인해주세요. 잠시 후 다시 시도해주세요.';
          } else if (checkErr.code === 'unauthenticated') {
      ...
      await updateDoc(doc(db, 'users', user.uid), { nickname: trimmedNickname });
  ...
  return (
    ...
    >
      ...
      {showAppInfo && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full modal-content">
            <h3 className="text-lg font-bold text-navy-900 mb-4">앱 정보</h3>
            <div className="space-y-3 mb-6">
              <div>
                <p className="text-sm font-semibold text-navy-700 mb-1">앱 이름</p>
                <p className="text-navy-900">RunLog</p>
              </div>
              <div>
                <p className="text-sm font-semibold text-navy-700 mb-1">버전</p>
                <p className="text-navy-900">v1.0.0</p>
              </div>
              <div>
                <p className="text-sm font-semibold text-navy-700 mb-1">설명</p>
                <p className="text-navy-600 text-sm">러닝 기록을 관리하는 앱입니다.</p>
              </div>
            </div>
            <button
              onClick={() => setShowAppInfo(false)}
              className="w-full px-4 py-2.5 bg-navy-700 text-white font-medium rounded-lg hover:bg-navy-800 transition-colors"
            >
              확인
            </button>
          </div>
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full modal-content">
            <h3 className="text-lg font-bold text-navy-900 mb-2">로그아웃</h3>
            <p className="text-navy-600 mb-6 text-sm">로그아웃하시겠습니까?</p>
            <div className="flex gap-2">
              <button
              ...
              <button
                ...
                className="flex-1 bg-navy-700 text-white py-2 rounded-lg hover:bg-navy-800 transition-colors text-sm font-semibold"
              >
                확인
              </button>
            </div>
