...
import { RACE_TYPES } from '../constants/races';
...
const AddRunForm = ({ user, onRunAdded, editingRun, onEditComplete, onClose }) => {
  ...
  useEffect(() => {
    if (editingRun) {
      ...
      // 관리자에서 등록한 대회 데이터와의 매칭 여부는
      // 폼 편집 시에는 모두 '직접 입력'으로 처리한다.
      const isKnownRace = false;
      const raceNameValue = 'CUSTOM';
      ...
      setFormData({
        ...
        customRaceName: editingRun.raceName || '',
  ...
  return (
    <>
      {isOpen && (
        <div 
          className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center p-0 sm:p-2 sm:p-4 z-[60]"
          onClick={handleClose}
        >
          <div 
            className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] sm:max-h-[90vh] flex flex-col modal-content"
            onClick={(e) => e.stopPropagation()}
            style={{ maxHeight: 'calc(100vh - env(safe-area-inset-top) - env(safe-area-inset-bottom) - 4rem)' }}
          >
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-3 sm:space-y-5 flex-1 overflow-y-auto pb-20 sm:pb-6">
              <div>
                      ...
                      >
                        ...
                        {!isExistingPhoto && (
                          ...
                          >
                            ...
                            <span className="hidden sm:inline">위치 조정</span>
                          </button>
                        )}
                        {/* 번호 표시 (좌측 상단) */}
            ...
            {/* 저장 버튼을 form 밖으로 분리하여 하단에 고정 */}
            <div className="sticky bottom-0 bg-white border-t border-navy-100 p-4 sm:p-6 pt-4" style={{ paddingBottom: 'max(5rem, calc(env(safe-area-inset-bottom) + 4.5rem))' }}>
              <button
                type="button"
                onClick={(e) => {
                  e.preventDefault();
